...
func TestRelay_PublishBatch_PublishesAndMarksRow(t *testing.T) {
	...
	consumer, err := js.CreateOrUpdateConsumer(ctx, broker.LedgerEventsStream, jetstream.ConsumerConfig{
		Durable:       "test-consumer",
		FilterSubject: "ledger.entry-recorded",
	})
	require.NoError(t, err)

	id := insertOutboxRow(t, db, "ledger.entry-recorded", []byte(`{"foo":"bar"}`))
