...
func ensureAccountsStream(t *testing.T, ctx context.Context, js jetstream.JetStream) {
	t.Helper()
...
func TestRelay_PublishBatch_PublishesAndMarksRow(t *testing.T) {
	...
	ensureAccountsStream(t, ctx, js)
	consumer, err := js.CreateOrUpdateConsumer(ctx, broker.AccountsEventsStream, jetstream.ConsumerConfig{
		Durable:       "test-consumer",
		FilterSubject: "account.updated",
	})
	require.NoError(t, err)

	id := insertOutboxRow(t, db, "account.updated", []byte(`{"foo":"bar"}`))
